...
#include <stdlib.h>
#include <string.h>
#include "pair_lj_class2_soft_coul_cut_nosoft.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairLJClass2SoftCoulCutNosoft::PairLJClass2SoftCoulCutNosoft(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
}
...
PairLJClass2SoftCoulCutNosoft::~PairLJClass2SoftCoulCutNosoft()
{
  if (allocated) {
...
void PairLJClass2SoftCoulCutNosoft::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	r2inv = 1.0/rsq;
        if (rsq < cut_coulsq[itype][jtype]) {
          ...
          //r2inv = 1.0/rsq;
          forcecoul = qqrd2e * qtmp*q[j]*sqrt(r2inv); //* r2inv;
        } else forcecoul = 0.0;
        ...
        fpair = factor_coul*forcecoul*r2inv + factor_lj*forcelj;
...
void PairLJClass2SoftCoulCutNosoft::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJClass2SoftCoulCutNosoft::settings(int narg, char **arg)
{
...
void PairLJClass2SoftCoulCutNosoft::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 7)
...
void PairLJClass2SoftCoulCutNosoft::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style lj/class2/soft/coul/cut/nosoft requires atom attribute q");
...
double PairLJClass2SoftCoulCutNosoft::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
    ...
    if (lambda[i][i] != lambda[j][j])
      error->all(FLERR,"Pair lj/class2/soft/coul/cut/nosoft different lambda values in mix");
...
void PairLJClass2SoftCoulCutNosoft::write_restart(FILE *fp)
{
...
void PairLJClass2SoftCoulCutNosoft::read_restart(FILE *fp)
{
...
void PairLJClass2SoftCoulCutNosoft::write_restart_settings(FILE *fp)
{
...
void PairLJClass2SoftCoulCutNosoft::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJClass2SoftCoulCutNosoft::write_data(FILE *fp)
{
...
void PairLJClass2SoftCoulCutNosoft::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJClass2SoftCoulCutNosoft::single(int i, int j, int itype, int jtype,
                                  double rsq,
                                  ...
                                  double &fforce)
{
  ...
  fforce = factor_coul*forcecoul*r2inv + factor_lj*forcelj;
...
void *PairLJClass2SoftCoulCutNosoft::extract(const char *str, int &dim)
{
  dim = 0;
