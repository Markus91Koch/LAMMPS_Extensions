...
void PairLJClass2Soft::compute_inner()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
...
void PairLJClass2Soft::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,forcelj,factor_lj,rsw;
...
void PairLJClass2Soft::compute_outer(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,forcelj,factor_lj,rsw;
...
void PairLJClass2Soft::init_style()
{
...
void PairLJClass2Soft::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
                         ...
                         double &fforce)
{
  ...
  if (rsq < cutsq[itype][jtype]) {
    philj = lj1[itype][jtype] * epsilon[itype][jtype] *
      (1.0/(denlj*denlj*denlj) - 1.0/(denlj*denlj)) - offset[itype][jtype];
  } else philj = 0.0;
