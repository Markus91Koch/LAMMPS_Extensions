...
#include <stdlib.h>
#include <string.h>
#include "pair_lj_class2_nosoft_coul_cut_soft.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairLJClass2NosoftCoulCutSoft::PairLJClass2NosoftCoulCutSoft(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
}
...
PairLJClass2NosoftCoulCutSoft::~PairLJClass2NosoftCoulCutSoft()
{
  if (allocated) {
...
void PairLJClass2NosoftCoulCutSoft::compute(int eflag, int vflag)
{
...
void PairLJClass2NosoftCoulCutSoft::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJClass2NosoftCoulCutSoft::settings(int narg, char **arg)
{
...
void PairLJClass2NosoftCoulCutSoft::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 7)
...
void PairLJClass2NosoftCoulCutSoft::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style lj/class2/nosoft/coul/cut/soft requires atom attribute q");
...
double PairLJClass2NosoftCoulCutSoft::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
    ...
    if (lambda[i][i] != lambda[j][j])
      error->all(FLERR,"Pair lj/class2/nosoft/coul/cut/soft different lambda values in mix");
  ...
  if (offset_flag) {
    ...
    double denlj = pow(cut_lj[i][j] / sigma[i][j], 3.0);
...
void PairLJClass2NosoftCoulCutSoft::write_restart(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoft::read_restart(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoft::write_restart_settings(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoft::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJClass2NosoftCoulCutSoft::write_data(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoft::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJClass2NosoftCoulCutSoft::single(int i, int j, int itype, int jtype,
                                  double rsq,
...
void *PairLJClass2NosoftCoulCutSoft::extract(const char *str, int &dim)
{
  dim = 0;
