...
#include <stdlib.h>
#include <string.h>
#include "pair_lj_class2_coul_cut_soft_test.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairLJClass2CoulCutSoftTest::PairLJClass2CoulCutSoftTest(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
}
...
PairLJClass2CoulCutSoftTest::~PairLJClass2CoulCutSoftTest()
{
  if (allocated) {
...
void PairLJClass2CoulCutSoftTest::compute(int eflag, int vflag)
{
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (rsq < cut_coulsq[itype][jtype]) {
          denc = sqrt(rsq);
          forcecoul = qqrd2e * qtmp*q[j] / (denc*denc*denc);
        } else forcecoul = 0.0;
        ...
        if (eflag) {
          if (rsq < cut_coulsq[itype][jtype])
            ecoul = factor_coul * qqrd2e * qtmp*q[j] / denc;
          else ecoul = 0.0;
          if (rsq < cut_ljsq[itype][jtype]) {
...
void PairLJClass2CoulCutSoftTest::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJClass2CoulCutSoftTest::settings(int narg, char **arg)
{
...
void PairLJClass2CoulCutSoftTest::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 7)
...
void PairLJClass2CoulCutSoftTest::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style lj/class2/coul/cut/soft/test requires atom attribute q");
...
double PairLJClass2CoulCutSoftTest::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
    ...
    if (lambda[i][i] != lambda[j][j])
      error->all(FLERR,"Pair lj/class2/coul/cut/soft/test different lambda values in mix");
...
void PairLJClass2CoulCutSoftTest::write_restart(FILE *fp)
{
...
void PairLJClass2CoulCutSoftTest::read_restart(FILE *fp)
{
...
void PairLJClass2CoulCutSoftTest::write_restart_settings(FILE *fp)
{
...
void PairLJClass2CoulCutSoftTest::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJClass2CoulCutSoftTest::write_data(FILE *fp)
{
...
void PairLJClass2CoulCutSoftTest::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJClass2CoulCutSoftTest::single(int i, int j, int itype, int jtype,
                                  double rsq,
                                  ...
                                  double &fforce)
{
  ...
  double eng = 0.0;
  if (rsq < cut_coulsq[itype][jtype]) {
    phicoul = force->qqrd2e *  atom->q[i]*atom->q[j] / denc;
...
void *PairLJClass2CoulCutSoftTest::extract(const char *str, int &dim)
{
  dim = 0;
