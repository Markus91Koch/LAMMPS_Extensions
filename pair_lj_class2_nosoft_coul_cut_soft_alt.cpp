...
#include <stdlib.h>
#include <string.h>
#include "pair_lj_class2_nosoft_coul_cut_soft_alt.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairLJClass2NosoftCoulCutSoftAlt::PairLJClass2NosoftCoulCutSoftAlt(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
}
...
PairLJClass2NosoftCoulCutSoftAlt::~PairLJClass2NosoftCoulCutSoftAlt()
{
  if (allocated) {
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
	  ...
	  rinv = sqrt(r2inv);
          r3inv = r2inv*rinv;
          r6inv = r3inv*r3inv;
	  forcelj = 18.0 * epsilon[itype][jtype] * r6inv *
            (lj2[itype][jtype] * r3inv - lj3[itype][jtype]) * r2inv;
        } else forcelj = 0.0;
        ...
        if (eflag) {
          ...
          if (rsq < cut_ljsq[itype][jtype]) {
            //evdwl = lj1[itype][jtype] * epsilon[itype][jtype] *
            //  (2.0/(denlj*denlj*denlj) - 3.0/(denlj*denlj)) - offset[itype][jtype];
            evdwl = epsilon[itype][jtype] * r6inv * (2.0 * lj2[itype][jtype] * r3inv - 3.0 * lj3[itype][jtype]) - offset[itype][jtype];
            evdwl *= factor_lj;
          } else evdwl = 0.0;
        }
...
void PairLJClass2NosoftCoulCutSoftAlt::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJClass2NosoftCoulCutSoftAlt::settings(int narg, char **arg)
{
...
void PairLJClass2NosoftCoulCutSoftAlt::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 7)
...
void PairLJClass2NosoftCoulCutSoftAlt::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style lj/class2/nosoft/coul/cut/soft/alt requires atom attribute q");
...
double PairLJClass2NosoftCoulCutSoftAlt::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
    ...
    if (lambda[i][i] != lambda[j][j])
      error->all(FLERR,"Pair lj/class2/nosoft/coul/cut/soft/alt different lambda values in mix");
  ...
  lj2[i][j] = pow(sigma[i][j],9.0); //pow(sigma[i][j], 3.0);
  lj3[i][j] = pow(sigma[i][j],6.0); //alphalj * (1.0 - lambda[i][j])*(1.0 - lambda[i][j]);
  lj4[i][j] = alphac  * (1.0 - lambda[i][j])*(1.0 - lambda[i][j]);


  if (offset_flag) {
    double denlj = lj3[i][j] + pow(cut_lj[i][j] / sigma[i][j], 3.0);
...
void PairLJClass2NosoftCoulCutSoftAlt::write_restart(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoftAlt::read_restart(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoftAlt::write_restart_settings(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoftAlt::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJClass2NosoftCoulCutSoftAlt::write_data(FILE *fp)
{
...
void PairLJClass2NosoftCoulCutSoftAlt::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJClass2NosoftCoulCutSoftAlt::single(int i, int j, int itype, int jtype,
                                  double rsq,
                                  ...
                                  double &fforce)
{
  ...
  if (rsq < cut_ljsq[itype][jtype]) {
    //r1sig3 = sqrt(rsq) / lj2[itype][jtype];
    //denlj = rsq*r1sig3;
    //forcelj = epsilon[itype][jtype] * 18.0 *
    //  (r1sig3/(denlj*denlj*denlj*denlj) - r1sig3/(denlj*denlj*denlj));
    rinv = sqrt(r2inv);
    r3inv = r2inv*rinv;
    r6inv = r3inv*r3inv;
  ...
  if (rsq < cut_ljsq[itype][jtype]) {
    ...
    //  offset[itype][jtype];
    philj = epsilon[itype][jtype] * r6inv * (2.0 * lj2[itype][jtype] * r3inv - 3.0 * lj3[itype][jtype]) - 
      offset[itype][jtype];
    eng += factor_lj*philj;
  }
...
void *PairLJClass2NosoftCoulCutSoftAlt::extract(const char *str, int &dim)
{
  dim = 0;
